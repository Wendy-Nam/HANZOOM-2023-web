            ...
            if ($user_id !== null && $username !== null) {
                ...
                if ($insertCommentResult) {
                    ...
                    // 추가한 댓글만을 가져오는 SQL 쿼리
                     $getAllCommentsSql = "SELECT comments.*, user.name AS username FROM comments
                     LEFT JOIN user ON comments.user_id = user.user_id
                     WHERE post_id = '$id' ORDER BY comment_date ASC";
                    $getAllCommentsResult = mysqli_query($conn, $getAllCommentsSql);

                    while ($newCommentRow = mysqli_fetch_array($getAllCommentsResult)) {
                        echo '<div class="comment">';
