...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['add_info'])) {
        ...
        $sql = "INSERT INTO user (sns_address, stay_duration, stay_purpose, gender) 
                VALUES ('$sns_address', '$stay_duration', '$stay_purpose', '$gender')";

        if (mysqli_query($conn, $sql)) {
            $message = "Additional info added successfully";
            $messageClass = "bg-green-100 border-green-400 text-green-800";
        } else {
            $message = "Error adding additional info: " . mysqli_error($conn);
            $messageClass = "bg-red-100 border-red-400 text-red-800";
        }
    }
}
...
if ($result) {
    ...
    <div class="container mx-auto p-10 bg-white shadow-md rounded-lg max-w-2xl mt-16">
        <h2 class="text-3xl font-semibold text-gray-800 mb-4">Add your information</h2>
        <!-- Display user information -->
        <!-- Update Profile Button -->
        <button class="btn btn-sm bg-yellow-300 border-2 border-yellow-400 mt-6 hover:border-yelllow-400 hover:bg-yellow-500" onclick="openAddInfoModal()">Add info</button>
    </div>
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
					...
					<div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <form action="user_info.php" method="post">
                                ...
                                <button type="submit" name="update_profile" class="btn btn-success mt-6 mb-6 btn-sm">Update Profile</button>
                            </form>
                        </div>
                    </div>
                </div>
...
<div id="add-info-modal" class="fixed inset-0 overflow-y-auto hidden">
    <!-- Background overlay with a semi-transparent gray color -->
    <div class="fixed inset-0 bg-gray-900 opacity-20"></div>
    <div class="flex items-center justify-center mt-36 md:mt-24">
        <!-- Add Info Form -->
        <div class="inline-block bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <button onclick="closeAddInfoModal()" class="btn fixed top-2 right-2 btn-ghost text-xl md:text-lg sm:text-base">X</button>
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    <h3 class="text-lg font-medium leading-6 text-gray-900 mb-6" id="modal-headline">
                        Add Additional Info
                    </h3>
                    <form action="user_info.php" method="post">
                        <div class="mb-4">
                            <label for="sns_address" class="block text-gray-700 text-sm font-bold mb-4">SNS Address</label>
                            <input type="text" id="sns_address" name="sns_address" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                            
                
                        </div>
                        <div class="mb-4">
                            <label for="stay_duration" class="block text-gray-700 text-sm font-bold mb-4">Stay Duration</label>
                            <input type="text" id="stay_duration" name="stay_duration" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        </div>
                        <div class="mb-4">
                            <label for="stay_purpose" class="block text-gray-700 text-sm font-bold mb-4">Stay Purpose</label>
                            <input type="text" id="stay_purpose" name="stay_purpose" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        </div>
                        <div class="mb-4">
                            <label for="gender" class="block text-gray-700 text-sm font-bold mb-4">Gender</label>
                            <input type="text" id="gender" name="gender" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        <!-- Include other fields for additional info similarly -->
                        <button type="submit"  name="add_info"class="btn btn-success mt-6 mb-6 btn-sm">Save</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    function closeAddInfoModal() {
        document.getElementById('add-info-modal').classList.add('hidden');
    }
